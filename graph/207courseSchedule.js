...
var canFinish = function (numCourses, prerequisites) {
    const noCycleExists = (graph, visited, currNode) => {
        ...
        for (const neighbor of neighbors) {
            ...
            //   console.log("Line 53");
            // }
            if (alreadyChecked.has(neighbor)) continue;
            if (graph.get(neighbor).has(currNode)) return false;
            if (visited.has(neighbor)) return false;
            didNotFindCycle = noCycleExists(graph, visited, neighbor);
        }
        // visited.delete(currNode);
        return didNotFindCycle;
    };

    let noCyclesFound = true;
    ...
    //   console.log("keys", graph.keys().next().value);
    ...
    for (let i = 0; i < graph.size; i++) {
        ...
        if (!alreadyChecked.has(key)) {
            console.log(key);
            noCyclesFound = noCycleExists(graph, visited, key);
        }
        if (!noCyclesFound) return noCyclesFound;
    }
